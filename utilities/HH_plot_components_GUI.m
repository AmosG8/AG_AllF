%%
function cellsToDel = plot_components_GUI(Y,A,C,b,currentFig,Cn,options, cellsToDel)
...
nb = size(currentFig,1);     % number of background components
...
Y_r = (AY- (A'*A)*C - full(A'*double(b))*currentFig) + C;

if plot_df
    [~,Df] = extract_DF_F(Y,[A,double(b)],[C;currentFig],size(A,2)+1);
else
...
% For R2014a and earlier: set(f,'Visible','on');
    ...
    function cellsToDel = surfzlim(source,callbackdata, cellsToDel)
        i = source.Value;
        plot_component(round(i))
        %%
        
        waitforbuttonpress;
        currentKey = get(fig,'CurrentCharacter');
            
        if strcmp(currentKey, 'a')
           cellsToDel(i, 1) = 1;
        end
        %%
...
%                 end
            ...
            plot(1:T,currentFig(i-nr,:)); title('Background activity','fontsize',16,'fontweight','bold');
            drawnow;
            if make_gif
                frame = getframe(fig); %getframe(1);
                im = frame2im(frame);
